...
// ){
//     try {
//         const [existingUser]= await db.select().from(Users).where(eq(Users.stripeCustomerId,clerkUserId))
//         .limit(1).execute()
//         if(existingUser){
//             const[updateUser] =await db.update(Users).set({name,email}).where((eq(Users.stripeCustomerId,clerkUserId)))
//             .returning().execute()
...
//     } catch (error) {
//         console.log(error,"error creating or updating user")
//         return null
